class Solution {
public:
    void back_tracking(vector<vector<char>>& board,
                       int curr,
                       int row_used[9][9],
                       int col_used[9][9],
                       int sub_used[9][9],
    ...
    {
        if(curr==board.size()*board.size()) res=board;
        else
        {
            int i=curr/9;
            int j=curr%9;
            ...
            if(board[i][j]=='.')
            {
                while(num<=8)
                {
                    if(row_used[i][num]||col_used[j][num]||sub_used[k][num])//num is impossible
                    {
                        ++num;
                        continue;
                    }
                    else
                    {
                        ...
                        col_used[j][num]=1;
                        sub_used[k][num]=1;
                        back_tracking(board,curr+1,row_used,col_used,sub_used,res);
                        row_used[i][num]=0;
                        col_used[j][num]=0;
                        sub_used[k][num]=0;
                        board[i][j]='.';
            ...
            {
                ++curr;
                back_tracking(board,curr,row_used,col_used,sub_used,res);
            }
        }
        return;
    }
    void solveSudoku(vector<vector<char>>& board)
    {
        ...
        vector<vector<char>> res;
        back_tracking(board,0,row_used,col_used,sub_used,res);
        board=res;
        return;
